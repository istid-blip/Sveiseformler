...
struct HeatInputView: View {
    ...
    // STOPWATCH STORAGE (Overlever app-lukking)
        ...
        GeometryReader { geometry in
            ZStack {
                    ...
                    VStack(spacing: 0) {
                        ...
                        HStack {
                            Button(action: { dismiss() }) {
                                Text("< MAIN MENU").font(RetroTheme.font(size: 16, weight: .bold)).foregroundColor(RetroTheme.primary).padding(8).overlay(Rectangle().stroke(RetroTheme.primary, lineWidth: 1))
                            }
                            Spacer(); Text("HEAT INPUT").font(RetroTheme.font(size: 16, weight: .heavy)).foregroundColor(RetroTheme.primary)
                        }.padding()
                ...
                if let target = focusedField {
                    VStack {
                        Spacer()
                        UnifiedInputDrawer(target: target, value: binding(for: target), range: range(for: target), step: step(for: target), isRecording: $isTimerRunning, onReset: resetStopwatch, onToggle: toggleStopwatch).padding(.bottom, 50)
                    }.id("DrawerContainer").transition(.move(edge: .bottom)).zIndex(100)
                }
            }
        }.navigationBarBackButtonHidden(true).onAppear { restoreActiveJob() }.crtScreen()
    ...
    @ViewBuilder func SelectableInput(label: String, value: Double, target: InputTarget, currentFocus: InputTarget?, precision: Int, action: @escaping () -> Void) -> some View {
        ...
        Button(action: { withAnimation(.spring(response: 0.3)) { action(); Haptics.selection() } }) {
            VStack(spacing: 0) {
                ...
                Text(label).font(RetroTheme.font(size: 10)).foregroundColor(RetroTheme.dim).padding(4)
            }
...
struct UnifiedInputDrawer: View {
    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 12).fill(Color.black).frame(width: 320, height: 280).overlay(RoundedRectangle(cornerRadius: 12).stroke(RetroTheme.dim, lineWidth: 1)).shadow(color: .black.opacity(0.8), radius: 15, x: 0, y: 15)
            ...
            if target == .time {
                VStack(spacing: 20) {
                    ...
                    Button(action: onToggle) {
                        ZStack {
                            ...
                            VStack(spacing: 2) {
                                if isRecording { Text(String(format: "%02d", Int(value))).font(.system(size: 50, weight: .black, design: .monospaced)).foregroundColor(.white).contentTransition(.numericText(value: value)); Text("SEC").font(RetroTheme.font(size: 10, weight: .bold)).foregroundColor(.white.opacity(0.8)) }
                ...
                .onChange(of: isRecording) { _, newValue in if newValue { withAnimation(.easeInOut(duration: 0.8).repeatForever(autoreverses: true)) { pulseAmount = 1.06 } } else { withAnimation(.spring()) { pulseAmount = 1.0 } } }
            } else {
                ...
                }.frame(width: 320, height: 280).clipShape(RoundedRectangle(cornerRadius: 12)).contentShape(Rectangle())
    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("\(job.passes.count) passes").font(RetroTheme.font(size: 10)).foregroundColor(RetroTheme.dim)
            }
            Spacer(); Text(job.date, format: .dateTime.day().month()).font(RetroTheme.font(size: 12)).foregroundColor(RetroTheme.dim)
